...
class Main:
    ...
    def print_available_emotions(self):
        emotions_str = ", ".join(
            f"(\033[0;91m{emotion.lower()}\033[0m)" for emotion in self.valid_emotions
        )
    ...
    def wait_for_tts_completion(self):
        ...
        while True:
            if self.tts_handler.sentence_queue.is_empty():
                finished_playout = (
                    not self.tts_handler.stream.is_playing()
                    and self.tts_handler.sentence_queue.is_empty()
                    and self.tts_handler.chunk_queue.empty()
                )
                if finished_playout:
